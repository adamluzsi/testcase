...
func TestRoundTripper(t *testing.T) {
	...
	s.Describe(".RoundTrip", func(s *testcase.Spec) {
		...
		s.When("net-timeout error is injected for a given service", func(s *testcase.Spec) {
			...
			s.Before(func(t *testcase.T) {
				r := request.Get(t)
				r = r.WithContext(faultinject.Inject(r.Context(), fihttp.TagTimeout(targetServiceName.Get(t))))
				request.Set(t, r)
			})
		...
		s.When("connection-refused error is injected for a given service", func(s *testcase.Spec) {
			...
			s.Before(func(t *testcase.T) {
				r := request.Get(t)
				r = r.WithContext(faultinject.Inject(r.Context(), fihttp.TagConnectionRefused(targetServiceName.Get(t))))
				request.Set(t, r)
			})
